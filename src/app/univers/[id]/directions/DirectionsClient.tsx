...
export default function DirectionsClient({
  ...
  const directions = React.useMemo<DirectionCard[]>(() => {
    if (directionsOverride && directionsOverride.length > 0) {
      return directionsOverride;
    }
    const baseTitle = titleOverride || universe.title || "Cet univers";
    return [
      {
        id: "dir-1",
        label: `${baseTitle} — Suite directe, plus mature`,
        format: "serie",
        tagline:
          "On reprend l'histoire là où elle s'est arrêtée, en assumant pleinement les conséquences et en élevant le ton dramatique.",
        emotionalTag: "Épique • Dramatique",
      },
      {
        id: "dir-2",
        label: `${baseTitle} — Préquelle centrée sur un personnage culte`,
        format: "miniserie",
        tagline:
          "On explore le passé d'un personnage clé, en révélant ce qui l'a façonné sans réécrire le canon principal.",
        emotionalTag: "Nostalgique • Intime",
      },
      {
        id: "dir-3",
        label: `${baseTitle} — Spin-off dans une zone inexplorée`,
        format: "serie",
        tagline:
          "Une nouvelle histoire autonome dans un coin encore inexploré de l'univers, respectant les règles établies.",
        emotionalTag: "Inspirant • Découverte",
      },
    ];
  }, [directionsOverride, titleOverride, universe.title]);
  ...
  const finalBackdrop =
    ...
    posterUrl ||
    placeholderPoster;
  const displayTitle = current?.titleOverride || titleOverride || universe.title;
        ...
        {!isFinished ? (
          <>
            ...
            {step === "poster" && (
              <>
                   <div
                     className="relative h-96 overflow-hidden rounded-2xl border border-zinc-900/70 bg-gradient-to-br from-zinc-800 via-zinc-900 to-black shadow-[0_40px_80px_rgba(0,0,0,0.8)] sm:h-[28rem]"
                     style={
                       finalBackdrop
                         ? {
                             backgroundImage: `linear-gradient(to top, rgba(0,0,0,0.8), transparent 55%), radial-gradient(circle at top left, rgba(248,113,113,0.35), transparent 55%), radial-gradient(circle at bottom right, rgba(24,24,27,0.95), transparent 55%), url(${finalBackdrop})`,
                  ...
                  <div className="absolute inset-0 flex flex-col justify-between p-6 sm:p-8">
                      ...
                      <div className="flex items-center justify-between text-xs sm:text-sm">
                        <span className="inline-flex items-center gap-1 rounded-full bg-black/70 px-3 py-1.5 font-semibold uppercase tracking-wide text-zinc-100 backdrop-blur-sm">
                            ...
                            <span className="text-sm text-zinc-300">
                             Direction {directionIndex + 1} / {directions.length}
                            </span>
                      </div>
                    ...
                    <div className="space-y-2 text-sm text-zinc-200">
                         ...
                         <p className="text-xs text-zinc-400">Univers : {displayTitle}</p>
                    </div>
                  </div>
                </div>
                <button
