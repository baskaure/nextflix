...
import type { UniverseStatistics } from "@/lib/statistics";

type BarometerClientProps = {
  universe: Universe;
...
export default function BarometerClient({
  universe,
  statistics,
  isFallback,
  ...
  const maxFormat = formatEntries.reduce((max, [key, value]) =>
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-[#050508] to-black text-zinc-100">
      ...
      <main className="mx-auto grid w-full max-w-7xl gap-6 px-5 pt-24 pb-16 sm:px-10 md:grid-cols-[3fr,2fr]">
        ...
        <section className="space-y-6 rounded-2xl border border-zinc-900/70 bg-gradient-to-br from-zinc-950/90 via-zinc-950/80 to-black p-6 shadow-xl shadow-black/60 sm:p-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-zinc-800/70 bg-zinc-900/60 p-4 backdrop-blur-sm">
              ...
              <p className="mt-2 text-2xl font-semibold text-zinc-50">
                {statistics.validatedDirectionShare}
                <span className="text-base text-zinc-500"> %</span>
              </p>
              <p className="mt-2 text-sm leading-relaxed text-zinc-400">
                valident au moins{" "}
            ...
            <div className="rounded-xl border border-zinc-800/70 bg-zinc-900/60 p-4 backdrop-blur-sm">
              ...
              <p className="mt-2 text-sm leading-relaxed text-zinc-400">
                ...
                <span className="font-medium text-zinc-100">{preferredFormatLabel}</span>{" "}
                {maxFormat[0] !== "aucun" && "plutôt qu'un autre format"}.
              </p>
            </div>
          </div>
        </section>
        ...
        <aside className="space-y-6 rounded-2xl border border-zinc-900/70 bg-gradient-to-br from-zinc-950/90 via-zinc-950/80 to-black p-6 shadow-xl shadow-black/60 sm:p-8">
          ...
          {statistics.topNonNegotiables.length > 0 && (
            <div className="space-y-3">
              <p className="text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-zinc-400">
                Doit absolument être respecté
              </p>
              <ul className="flex flex-wrap gap-2">
                {statistics.topNonNegotiables.map((item) => (
                  <li
                    key={item.label}
          ...
          {statistics.dominantFeeling && (
            <div className="space-y-3">
              <p className="text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-zinc-400">
                Sentiment dominant attendu
              </p>
              <p className="text-sm font-medium text-zinc-50">
                {FEELING_LABELS[statistics.dominantFeeling] || statistics.dominantFeeling}
              </p>
              <p className="text-[0.8rem] text-zinc-400">
                Les fans attendent une suite qui provoque surtout ce type de{" "}
                <span className="font-medium text-zinc-100">ressenti émotionnel</span>.
              </p>
            </div>
          )}
          ...
          <div className="mt-2 space-y-2 rounded-xl border border-zinc-800/70 bg-black/50 p-4 backdrop-blur-sm">
            ...
            <p className="text-sm leading-relaxed text-zinc-300">
              {statistics.signatureCount > 0 ? (
                <>
                  ...
                  {statistics.rejectRebootShare > 0 && (
                    <>
                      {" "}
                      • {statistics.rejectRebootShare}%{" "}
                      <span className="font-medium text-zinc-100">refusent un reboot</span>
                    </>
                  )}
                  {statistics.validatedDirectionShare > 0 && (
                    <>
                      {" "}
                      • {statistics.validatedDirectionShare}%{" "}
                      <span className="font-medium text-zinc-100">valident une direction</span>
                    </>
                  )}
                  {maxFormat[1] > 0 && (
                    <>
                      {" "}
                      • {maxFormat[1]}% préfèrent le format{" "}
                      <span className="font-medium text-zinc-100">{preferredFormatLabel}</span>
                    </>
                  )}
                  .
                ...
                <>
                  Aucune donnée disponible pour le moment. Les statistiques apparaîtront
                  lorsque les premiers utilisateurs auront donné leur avis.
                </>
              )}
            </p>
          </div>
